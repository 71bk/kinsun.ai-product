async (page) => {
  ...
  for (const [locale,width,height] of [['zh-Hant',768,1024],['zh-Hant',1440,900],['zh-Hant',375,812],['zh-Hant',390,844],['zh-Hant',430,932],['en',390,844],['en',768,1024]]) {
    ...
    await page.goto(base+'/staff?qa=workbench-'+locale+'-'+width);
    ...
    await page.screenshot({path:folder+`workbench-schedule-${locale}-${width}.png`,fullPage:true});
    await link.click();
    ...
    await page.screenshot({path:folder+`workbench-handover-${locale}-${width}.png`,fullPage:true});
    ...
    await page.screenshot({path:folder+`workbench-confirm-${locale}-${width}.png`});
    ...
    await page.screenshot({path:folder+`workbench-completed-${locale}-${width}.png`,fullPage:true});
