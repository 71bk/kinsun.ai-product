  ...
  items: [
    {
      ...
      display_name: 'Synthetic Future Elder',
      status: 'CONFIRMED',
      scheduled_start: '2026-09-09T02:00:00Z',
      scheduled_end: '2026-09-09T03:00:00Z',
...
function mount(locale: 'en' | 'zh-Hant' = 'en', onAccessCheck = vi.fn()) {
  return render(
    createElement(LocaleProvider, {
      ...
      children: createElement(HomeCareSchedule, { config, onAccessCheck }),
    }),
  );
}
afterEach(() => {
...
it.each(['en', 'zh-Hant'] as const)(
  'links only to the exact assignment without elder detail or commands: %s',
  async (locale) => {
    load.mockResolvedValue(fixture());
    mount(locale);
    await screen.findByText('Synthetic Future Elder');
    expect(screen.getByRole('link').getAttribute('href')).toBe(
      '/staff/assignments/synthetic-assignment',
    );
...
it('clears expired rows and reloads using server-relative time', async () => {
  ...
  const data = fixture();
  data.items[0].scheduled_end = '2026-09-09T01:00:01Z';
...
it('replaces pagination instead of retaining possibly revoked earlier rows', async () => {
  load.mockResolvedValueOnce({
    ...fixture(),
    page: { next_cursor: 'opaque', has_more: true, limit: 20 },
  });
  mount();
  await screen.findByText('Synthetic Future Elder');
  load.mockResolvedValue({ ...fixture(), items: [] });
...
it('reloads authoritative schedule state after a visit update', async () => {
  load.mockResolvedValueOnce(fixture()).mockResolvedValue({ ...fixture(), items: [] });
  mount();
  await screen.findByText('Synthetic Future Elder');
