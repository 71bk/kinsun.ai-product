...
import { watchAssignmentUpdates } from '@/lib/assignment-updates';
import { useLocale } from '@/lib/i18n/locale-context';
import styles from './HomeCareSchedule.module.css';

export function HomeCareSchedule({
  config,
  onAccessCheck,
}: {
  config: ApiConfig;
  ...
  const load = useCallback(
    (cursor?: string) => {
      ...
      setPage(null);
      setError(false);
      void getHomeCareSchedule(config, cursor)
        .then((result) => {
          ...
          setPage({
            ...result,
            items: result.items.filter((row) => Date.parse(row.scheduled_end) > serverNow),
          });
        })
        .catch((caught) => {
  ...
  useEffect(() => {
    ...
    const focus = () => load();
    const unwatch = watchAssignmentUpdates(focus);
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(
      () => load(),
      Math.max(0, Math.min(30_000 - elapsed, end - serverNow)),
    );
  ...
  return (
    <section className={styles.section} aria-labelledby="home-schedule-heading">
      ...
      {error ? (
        <ErrorState description={t('error.loadAssignmentsFailed')} />
      ) : !page ? (
        <Skeleton rows={2} />
      ) : (
        <>
          ...
          <div className={styles.list}>
            {page.items.map((row) => {
              ...
              return (
                <article className={styles.card} key={row.assignment_id}>
                  <h3>{row.display_name}</h3>
                  <p className={styles.status}>
                    <Icon aria-hidden="true" size={20} weight="bold" />
                    {t(row.status === 'CONFIRMED' ? 'schedule.confirmed' : 'schedule.inProgress')}
                  </p>
                  <p>
                    <time dateTime={row.scheduled_start}>{format(row.scheduled_start)}</time> –{' '}
                    <time dateTime={row.scheduled_end}>{format(row.scheduled_end)}</time>
                  </p>
                  <p>
                  ...
                  <Link
                    ...
                    href={`/staff/assignments/${encodeURIComponent(row.assignment_id)}`}
                  >
                    {t('workbench.open')}
                  </Link>
                </article>
          ...
          {page.page.has_more && (
            <button
              className={styles.button}
              type="button"
              onClick={() => load(page.page.next_cursor ?? undefined)}
            >
              {t('schedule.next')}
            </button>
          )}
