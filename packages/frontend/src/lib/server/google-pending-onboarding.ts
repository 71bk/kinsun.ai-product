...
import type { PendingGoogleCoreHandoff } from './google-oidc-core-handoff';
import type { GoogleOidcTransaction } from './google-oidc-transaction';

const DEVELOPMENT_COOKIE = 'kinsun_google_pending_onboarding';
const PRODUCTION_COOKIE = '__Host-kinsun_google_pending_onboarding';
...
function signingSecret(): string {
  ...
  if (!secret || Buffer.byteLength(secret, 'utf8') < 32) {
    throw new Error('Google pending onboarding is unavailable');
  }
  return secret;
}
...
export function createGooglePendingOnboarding(
  handoff: PendingGoogleCoreHandoff,
  transaction: GoogleOidcTransaction,
): GooglePendingOnboarding {
  if (transaction.intent === 'STAFF') throw new Error('Invalid Google onboarding intent');
  const now = Date.now();
  ...
  if (!Number.isFinite(expiresAt) || expiresAt <= now || expiresAt - now > MAX_TTL_SECONDS * 1000) {
    throw new Error('Invalid Google pending expiry');
  }
  return {
    createdAt: now,
    expiresAt,
...
export function serializeGooglePendingOnboarding(value: GooglePendingOnboarding): string {
  const payload = Buffer.from(JSON.stringify(value), 'utf8').toString('base64url');
  return `${payload}.${signature(payload)}`;
}

export function parseGooglePendingOnboarding(
  value: string | undefined,
): GooglePendingOnboarding | null {
...
export function googlePendingOnboardingCookieName(): string {
  return process.env.NODE_ENV === 'production' ? PRODUCTION_COOKIE : DEVELOPMENT_COOKIE;
}

export function googlePendingOnboardingCookieOptions(maxAge = MAX_TTL_SECONDS) {
  return {
    httpOnly: true,
