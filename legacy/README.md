...
可用 `git log --follow` 查）。ADR 0007 已將它與現有 `infra/lib/elderly-care-stack.ts`
一併定為 legacy：不加入新功能，也不得部署現有 Lambda／DynamoDB／另一套 Cognito stack。
一般 HTTP 主線只走 Next.js BFF → Python Core → Agent Runtime。

搬出 npm workspaces（根 `package.json` 的 `"workspaces": ["packages/*", "infra"]`
...
要看這段程式碼在凍結前的完整歷史、或它與 `packages/shared`／`infra/` 曾經如何
