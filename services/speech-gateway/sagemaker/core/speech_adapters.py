...
class MockASRAdapter(ASRAdapter):
    ...
    def transcribe(
        ...
        return ASRResult(
            ...
            needs_confirmation=requires_confirmation(transcript, confidence, context.key_entities),
        )
...
class TransformersASRAdapter(ASRAdapter):
    ...
    def transcribe(
        ...
        return ASRResult(
            ...
            latency_ms=latency,
            realtime_factor=(latency / 1000 / duration) if duration else None,
            needs_confirmation=requires_confirmation(transcript, None, context.key_entities),
        )
...
class CTranslate2ASRAdapter(ASRAdapter):
    ...
    def transcribe(
        ...
        inference_language = (
            "zh" if requested_language in {"nan", "hak", "mixed-zh-nan"} else requested_language
        )
        ...
        scored_segments = [
            segment for segment in segments if getattr(segment, "avg_logprob", None) is not None
        ]
        if scored_segments:
            weights = [
                max(0.01, float(segment.end) - float(segment.start)) for segment in scored_segments
            ]
            confidence = sum(
...
class VoxHakkaAdapter(TTSAdapter):
    ...
    def __init__(self, venv_python: Path | None = None, dialect: str = "sixian"):
        ...
        self.venv_python = venv_python or (local_poc / ".venv-voxhakka" / "Scripts" / "python.exe")
