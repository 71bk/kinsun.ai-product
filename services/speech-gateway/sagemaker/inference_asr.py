...
HTTP contract used by the canonical `speech_gateway.sagemaker_asr` adapter
(see `docs/sagemaker-endpoint-contract.md`).
...
port 8080. `CustomAttributes` from the gateway's InvokeEndpoint call
...
# Canonical gateway locale codes -> the inference core's bare language codes.
# Only nan/hak reach this endpoint; zh-TW/en-US use Amazon Transcribe.
...
def _get_adapter() -> tuple[Any, Any]:
    ...
    if _adapter is None:
        ...
        model_id = os.environ.get("KINSUN_ASR_MODEL_ID", "adi-gov-tw/Taiwan-Tongues-ASR-CE-v2.0")
...
def invocations() -> Response:
    ...
    response = Response(json.dumps(payload, ensure_ascii=False), mimetype="application/json")
