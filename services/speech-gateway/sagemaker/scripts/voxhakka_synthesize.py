...
def load_model(model_id: str = MODEL_ID) -> Synthesizer:
    ...
    model_dir = os.environ.get("KINSUN_HAK_TTS_MODEL_PATH") or snapshot_download(model_id)
    ...
    content = content.replace("speaker_embs.pth", speaker_embedding_file_path.replace("\\", "/"))
...
def synthesize(
    ...
    if result.unknown_words:
        raise ValueError(f"Words could not be converted to IPA: {', '.join(result.unknown_words)}")
