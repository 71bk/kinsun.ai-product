...
`docs/model-selection.md`) behind the exact HTTP contract used by the canonical
`speech_gateway.sagemaker_tts` adapter (see
`docs/sagemaker-endpoint-contract.md`).
...
# Matches the canonical Polly adapter's SSML_RATE mapping (80%/100%/120%) so
# "slow"/"normal"/"fast" means the same thing
...
def synthesize_bytes(text: str, language: str, speaking_speed: str) -> bytes:
    ...
    Deliberately does not catch adapter exceptions. Speech Gateway turns a
    failed endpoint invocation into a safe 502; its caller retains Core's text
    reply as the explicit fallback.
    """
