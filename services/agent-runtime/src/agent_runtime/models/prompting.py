...
嚴格規則：
1. 不提供醫療診斷、治療建議或用藥建議。
2. 只能引用系統提供的已授權記憶；本人自述的偏好不是已核實照護紀錄，不要編造過去的對話內容。
...
8. 只輸出要直接對長者說的最終回覆。不要輸出推理過程、規則檢查、評分、編號清單或英文自我評估。
   是否保存記憶由系統另外顯示，不要自行宣稱已保存。沒有提供的記憶就坦白說不知道。
   日常習慣不能證明某一天實際做過什麼；例如每天喝豆漿不代表昨天一定喝了豆漿。"""
...
def _build_user_prompt(
    ...
    if not excerpts:
        ...
        return (
            address_section + "以下內容是已授權的記憶、人工覆核事件或具來源的照護資料，"
            "請保留各項來源差異，"
