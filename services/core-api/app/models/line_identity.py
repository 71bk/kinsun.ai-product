...
Only keyed digests are persisted. Raw LINE user IDs, link tokens, nonces,
reply tokens, and message text never enter these tables.
"""
...
class LineLinkChallenge(BaseModel, TenantScopedMixin, VersionedMixin):
    """Short-lived nonce binding a Cognito-authenticated elder to LINE."""
...
class LineWebhookReceipt(BaseModel):
    ...
    __table_args__ = (
        ...
        sa.UniqueConstraint(
            "webhook_event_id",
            name="uq_line_webhook_receipt_event_id",
        ),
