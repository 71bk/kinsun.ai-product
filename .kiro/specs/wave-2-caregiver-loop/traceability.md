...
### B04 來源篩選後端增量（2026-09-15）

新增明確來源寫入與 MANUAL／CONVERSATION_SESSION／UNKNOWN 查詢，與既有日期／類型／
status 條件一起在分頁前套用。保留來源未知的歷史 row，不回填猜測的 MANUAL；
migration `a5c7e9f1b324` 接續 B03 的 `f4b6d8e0a213`。
...
PR #46 的 `44d87ca` 已通過 CI `34830767622` 全部 10 jobs，20 migration／212 integration
（新增 8）；這次無新的 real-auth 或 production deployment 證據。
...
- 派案服務紀錄 Core API 已隨 PR #38 合併（`9caf7cc`）；修正 fixture lifecycle 後 PR CI `34316911448`／main CI `34317304938` 全過，20 migration／168 integration 通過。Supabase development 已 additive 升級至 `e3a5c7d9f102`，無 backfill／scope grant；runtime least-privilege 尚未啟用。見 [Service record report](../../../docs/project/assignment-service-record-20260909.md)。
- 服務紀錄 UI 入口、二次確認、不可變唯讀、same-key retry 與 access-loss 清除已隨 PR #39 合併；原切片驗證見 [UI report](../../../docs/project/service-record-entry-ui-20260909.md)。
- 2026-09-11 可選「提交紀錄並完成服務」已完成本機 API／UI／契約與 synthetic Browser QA；Core unit 1,359、Frontend 全套 544（最後相關回歸 41）、Agent schema 260 通過。PR #42 已於 2026-09-14 合併（`4c56035`）；PR CI `34564617822` 與 main CI `34796372613` 全部 10 jobs 成功，包含新增 9 DB rollback／concurrency／scope cases（完整 DB gate：20 migration／177 integration）。尚未部署，無此 UI 的 real-auth write E2E。見 [Completion report](../../../docs/project/service-record-completion-20260911.md)。
- 2026-09-14 Owner 核准「上次服務紀錄」跨 worker 第一切片：US-C01／US-C02／NFR-H01，當次有效 IN_PROGRESS 本人派案＋獨立 history scope，來源同 tenant／care unit／elder、COMPLETED、不重疊、正式 v1；deletion request／tombstone 保守排除。Core／Frontend／strict contract 已實作，本機 Core unit 1,375、Frontend 571、static／Core live contract 93 operations 通過；新增 DB cases 與 Browser QA 結果見 [交付紀錄](../../../docs/project/previous-service-record-20260914.md)。未授予 development scope、未宣稱 real-auth E2E 或部署。
- 同日 [PR #44](https://github.com/71bk/kinsun.ai-product/pull/44) 的程式基準 `f4b2847` 已通過 [CI 34811244713](https://github.com/71bk/kinsun.ai-product/actions/runs/34811244713) 全部 10 jobs：Core unit 1,375、Frontend 572、20 migration／204 integration（新增 27）及其他服務／契約／cross-service gate。14 組 synthetic Browser QA 截圖與 DOM 全過。第一輪的 membership fixture 期間錯誤已修正，保留失敗 run 34810612170。
- PR #44 其後已合併為 `4556e57`，main run `34813366993` 成功。同日另以兩個全新 synthetic 帳號完成真實登入、跨 worker 交接、缺 scope／跨 tenant／他人／舊派案 404、UI 提交並完成 201 與 DB／兩筆 outbox 核對，最後 writer 撤權後 401 清除所有卡片與原文。全部 campaign 授權／憑證已撤銷，取代前述「無 real-auth」的階段狀態；未部署。見 [Real-auth report](../../../docs/project/previous-service-record-real-auth-20260914.md)。該次環境診斷的 development 驗證設定輪替仍待 Owner 決定，不視為結案。
