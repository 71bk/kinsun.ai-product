...
- [ ] 2. 完成 US-F02 候選行動建議
  - [ ] 2.1 定義 Candidate schema、狀態、reason/source/due 與 reject/exclude reason
  - [ ] 2.2 實作 deterministic allowed-action／medical-boundary gate
  - [ ] 2.3 實作 proposal-only Agent path，不允許直接 formal write
  - [ ] 2.4 實作照護者採納／拒絕 UI，採納後才呼叫 R1 formal create command
  - [ ] 2.5 補 contract、cross-scope、zero-side-effect、red-team 與 visual evidence
  - _Requirements: R2_
