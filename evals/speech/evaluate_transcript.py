        ...
        for hypothesis_index, hypothesis_char in enumerate(hypothesis, start=1):
            current.append(
                min(
                    ...
                    previous[hypothesis_index - 1] + (reference_char != hypothesis_char),
                )
            )
        previous = current
    return previous[-1]
...
def evaluate_case(case: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "normalized_hanji_cer": character_error_rate(normalized_reference, normalized_hypothesis),
        "tailo_cer": tailo_cer,
...
def write_table_reports(results: list[dict[str, Any]], json_output: Path) -> tuple[Path, Path]:
...
def main() -> int:
    ...
    print(f"evaluated {len(results)} synthetic cases -> {args.output}, {csv_path}, {html_path}")
    return 0
