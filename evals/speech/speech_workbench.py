...
SPEECH_GATEWAY_SRC = REPOSITORY_ROOT / "services" / "speech-gateway" / "src"
if str(SPEECH_GATEWAY_SRC) not in sys.path:
    sys.path.insert(0, str(SPEECH_GATEWAY_SRC))

from speech_gateway.asr import MODEL_VERSION as TRANSCRIBE_MODEL_VERSION  # noqa: E402
from speech_gateway.asr import transcribe_pcm  # noqa: E402
...
def invoke_asr(
    ...
    try:
        ...
        if language in LOW_RESOURCE_LANGUAGES:
            ...
            transcript, confidence, _segments = asyncio.run(
                transcribe_pcm(pcm_bytes, language, 16000, region)
            )
            payload = {"text": transcript, "confidence": confidence}
            provider = "amazon-transcribe-streaming"
            model_version = TRANSCRIBE_MODEL_VERSION
    ...
    metadata = {
        "provider": provider,
        "endpoint": endpoint_name.strip() if language in LOW_RESOURCE_LANGUAGES else None,
        "language": language,
...
def run_mock_asr(mock_text: str, mock_confidence: float) -> tuple[str, float, str, dict[str, Any]]:
...
def synthesize_external_tts(
    ...
    try:
        if language == "hak-TW":
            segmented, romanization, audio_path = Client("ivanusto/tw-hakka-tts").predict(
...
def build_demo() -> gr.Blocks:
    with gr.Blocks(title="Kinsun Speech 整合工作台") as demo:
        ...
        with gr.Tab("1. ASR 與低信心確認"):
            with gr.Row():
                ...
                with gr.Column():
                    ...
                    asr_approved_data = gr.Checkbox(label="我確認音訊為 Synthetic／核准去識別資料")
                    asr_run = gr.Button("執行 AWS ASR", variant="primary")
            with gr.Accordion("Mock 模式（不呼叫 AWS）", open=False):
                mock_text = gr.Textbox(value="這是一段 Synthetic 測試逐字稿。", label="Mock 逐字稿")
                mock_conf = gr.Slider(0, 1, value=0.5, step=0.01, label="Mock confidence")
        ...
        with gr.Tab("2. Core → Agent／RAG"):
            gr.Markdown("必須先在上一頁確認逐字稿；正式權限與 Consent 由 Core 重新檢查。")
        ...
        with gr.Tab("3. TTS 與播放"):
            ...
            with gr.Row():
                ...
                hakka_dialect = gr.Dropdown(HAKKA_DIALECTS, value="sixian", label="客語腔調")
                speed = gr.Slider(0.5, 1.5, value=1.0, step=0.1, label="外部客語語速（越大越慢）")
            ...
            with gr.Accordion("SageMaker TTS 設定", open=False):
                ...
                speed_label = gr.Dropdown(["slow", "normal", "fast"], value="normal", label="語速")
...
def main() -> None:
    ...
    build_demo().launch(server_name="127.0.0.1", server_port=port, share=False, show_error=False)
